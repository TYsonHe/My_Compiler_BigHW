	...
	if (productions[item.pro].right.size() == item.pointPos) {
		...
		for (iter = productions.begin(); iter < productions.end(); iter++) {
			...
			if (iter->left == productions[item.pro].right[item.pointPos]) {
				// 将产生式的派生加入I中
				int index = static_cast<int>(iter - productions.begin());
				I temp = derive(Item{ index,0 });
...
void ParserAndSemanticAnalyser::createDFA() {
	...
	int nowI = 0; // 当前状态的编号
	dfa.stas.push_back(derive(Item{ 0,0 })); // 生成一个初始的项目集I（即DFA的一个状态S）
	// 遍历每一个状态
	for (list<I>::iterator iter = dfa.stas.begin(); iter != dfa.stas.end(); iter++, nowI++) {
		// 遍历状态的每一个项目
		for (set<Item>::iterator itIter = iter->items.begin(); itIter != iter->items.end(); itIter++) {
			...
			if (productions[itIter->pro].right.size() == itIter->pointPos) {
				set<Symbol>FOLLOW = follow[productions[itIter->pro].left]; // 取FOLLOW集合
				for (set<Symbol>::iterator followIter = FOLLOW.begin(); followIter != FOLLOW.end(); followIter++) {
					if (LR1_Table.count(GOTO(nowI, *followIter)) == 1) {
						string err = "文法不是LR1文法，移进规约冲突";
						outputError(err);
					}
					if (itIter->pro == 0) {
						LR1_Table[GOTO(nowI, *followIter)] = Behavior{ accept,itIter->pro };
					}
					else {
						LR1_Table[GOTO(nowI, *followIter)] = Behavior{ reduct,itIter->pro };
					}
...
void ParserAndSemanticAnalyser::printLRTable(const char* fileName) {
	ofstream fout;
	fout.open(fileName);
	if (!fout.is_open()) {
		outputError("文件" + string(fileName) + "打开失败");
...
void ParserAndSemanticAnalyser::printLRTable(ostream& out)
{
	for (const auto& entry : LR1_Table) {
		int state = entry.first.first;
		Symbol symbol = entry.first.second;
		Behavior behavior = entry.second;

		out << "State: " << state << ", Symbol: " << symbol.content << " -> ";

		switch (behavior.behavior) {
		case shift:
			out << "Shift, Next State: " << behavior.nextStat << endl;
			break;
		case reduct:
			out << "Reduce, Production ID: " << behavior.nextStat << endl;
			break;
		case accept:
			out << "Accept" << endl;
			break;
...
bool ParserAndSemanticAnalyser::match(list<string>& argument_list, list<DType>& parameter_list) {
	// 匹配参数列表个数
...
void ParserAndSemanticAnalyser::pushSymbol(Symbol* sym) {
	symStack.push(sym);
	if (LR1_Table.count(GOTO(staStack.top(), *sym)) == 0) {
		outputError(string("语法错误：第") + to_string(lineCount) + "行，不期待的符号" + sym->content);
	}
	Behavior bh = LR1_Table[GOTO(staStack.top(), *sym)];
...
void ParserAndSemanticAnalyser::analyse(list<Token>& words, ostream& out) {
	...
	for (list<Token>::iterator iter = words.begin(); iter != words.end(); ) {
		...
		// 检查是否有对应的 GOTO 表项
		if (LR1_Table.count(GOTO(staStack.top(), *nextSymbol)) == 0) {
			outputError(string("语法错误：第") + to_string(lineCount) + "行，不期待的符号" + nextSymbol->content);
		}

		// 获取行为
		Behavior bh = LR1_Table[GOTO(staStack.top(), *nextSymbol)];
		if (bh.behavior == shift) {
			...
			case 3:
			{
				...
				// 处理函数声明 int ID M A function_declare
			...
			case 5:
			{
				...
				// 处理 void ID M A function_declare
				FunctionDeclare* function_declare = (FunctionDeclare*)popSymbol();
				Symbol* A = popSymbol();
				M* m = (M*)popSymbol();
				Id* ID = (Id*)popSymbol();
			...
			case 8:
			{
				...
				// 处理函数定义 ( parameter ) sentence_block
			...
			case 9:
			{
				...
				// 处理参数定义 parameter = parameter_list
			...
			case 12:
			{
				// parameter_list1 ::= param , parameter_list2
				// 处理多个参数 param , parameter_list
			...
			case 14:
			{
				...
				// 处理代码块 { inner_declare sentence_list }
			...
			case 18:
			{
				...
				// 处理语句列表 sentence M sentence_list
			...
			case 29:
			{
				// if_sentence ::= if ( expression ) A1 sentence_block1 N else M A2 sentence_block2
			...
			case 33:
			{
				// expression ::= add_expression1 > add_expression2
			...
			case 40:
			{
				// add_expression1 ::= item + add_expression2
				AddExpression* add_expression2 = (AddExpression*)popSymbol();
			...
			case 47:
			{
				...
				if (!f) {
				...
				else if (!match(argument_list->alist, f->paramTypes)) {
					...
					// 为函数调用生成中间代码
					factor->name = nt.newTemp(); // 生成临时变量名
					code.emit("call", ID->name, "_", "_"); // 生成函数调用指令
					code.emit("=", "@RETURN_PLACE", "_", factor->name); // 将返回值赋值给临时变量


					pushSymbol(factor); // 将生成的 Factor 符号压入符号栈
				}
				break;
			}
			case 48:
			...
			case 51:
			{
				// argument_list1 ::= expression , argument_list2
